...
function getFallbackTriage(inputText: string, targetLang: string): DistressTriageResult {
  ...
  const isPoisonBite = /poison|zahar|snake\s*bite|saanp|dog\s*bite|kutta|rabies|toxic|chemical|जहर|विष|सांप\s*काटना|कुत्ता\s*काटना/i.test(text);
  ...
  const isLevel1 = isChestCardiac || isBreathing || isStrokeNeuro || isMajorTraumaBleeding || isPoisonBite || isSevereBurn || isPediatricCritical;
  const isLevel2 = isFever || isAbdominal || isFractureMinorTrauma || isHeadacheDizzy;
  const level: TriageLevel = isLevel1 ? 1 : (isLevel2 ? 2 : 3);
  const category = level === 1 ? 'Immediate / Critical' : (level === 2 ? 'Urgent' : 'Non-Urgent');
  ...
  if (isPoisonBite) keySignals.push('Toxic exposure / venomous bite protocol');
  ...
  const suggestedFacilityType = level === 1
    ? (isChestCardiac ? 'Super-Specialty Cardiac & Cath Lab Emergency Center' : 'Super-Specialty Level-1 Trauma & Critical Care Center')
...
// Helper function to call Gemini with automatic model cascade (gemini-flash-latest -> gemini-3.8-flash)
...
1. NON-DIAGNOSTIC REQUIREMENT: You must NOT provide a definitive clinical medical diagnosis. Instead, accurately assess urgency triage level:
   - Level 1: Immediate / Critical (Life-threatening: chest pain/suspected heart attack, acute respiratory failure, stroke signs, unconsciousness, heavy bleeding, major accident trauma, venomous snakebite, severe poisoning)
   - Level 2: Urgent (Prompt medical examination needed: fever with chills/body ache, acute abdominal pain, suspected bone fracture, deep lacerations, vomiting/diarrhea with dehydration, asthma flare-up, dog bite)
   - Level 3: Non-Urgent (Mild illness, cold, routine cough, mild sprain, chronic rash, general checkup)
2. MULTILINGUAL & CODE-MIXING SUPPORT: The user input may be in Hindi (Devanagari or Romanized), English, Tamil, Telugu, Marathi, or colloquial Hinglish (e.g. "Mere father ko chest pain hai and saans lene mein dikkat ho rahi hai" or "mujhe bukhaar h" or "मुझे तेज बुखार है").
...
function getFallbackAIAnalysis(
  ...
  const urgencyLabel = urgencyLevel === 1 ? 'Level 1: Immediate / Critical' : (urgencyLevel === 2 ? 'Level 2: Urgent' : 'Level 3: Non-Urgent');
  ...
  if (isPoisonBite) complaints.push('Toxic exposure / suspected venomous envenomation');
